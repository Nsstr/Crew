    ...
    function renderDesktopView() {
      ...
      // Define header columns HTML for the modular tables
      ...
      days.forEach(d => {
        ...
        let thClass = isToday ? 'dia-actual' : '';
        if (isHoliday) thClass += ' holiday-col';
        
        const thBg = evento ? `background-color: ${evento.color}22;` : '';
        const holidayBadge = isHoliday ? `<span class="holiday-badge">Feriado</span>` : '';
        const eventBadge = evento ? `<span class="event-badge" style="background-color:${evento.color};" title="${evento.descripcion}">&#128197; ${evento.descripcion}</span>` : '';
        theadHTML += `<th class="${thClass}" style="text-align: center; ${thBg}">${dayName} ${holidayBadge}${eventBadge}</th>`;
      });
      theadHTML += `</tr>`;

      const modularGrid = document.getElementById('modularGridContainer');
      if (modularGrid) modularGrid.innerHTML = ''; // Limpiar grilla anterior
      ...
      areasOrderDesktop.forEach(areaName => {
         ...
         metricRow.style.background = 'rgba(255, 255, 255, 0.05)';
         metricRow.style.borderBottom = '2px solid var(--border)';
         
         const metricLabel = document.createElement('td');
         metricLabel.textContent = 'Cobertura';
         metricLabel.style.fontWeight = 'bold';
         metricLabel.style.fontSize = '0.8rem';
         metricLabel.style.color = 'var(--text-muted)';
         ...
         days.forEach(d => {
             ...
             td.textContent = presentes > 0 ? presentes : '-';
         ...
         const francoLabelCell = document.createElement('td');
         francoLabelCell.textContent = 'Total Francos';
         francoLabelCell.style.fontWeight = 'bold';
         francoLabelCell.style.color = '#a855f7';
         francoLabelCell.style.fontSize = '0.8rem';
         francoLabelCell.style.padding = '2px 0 2px 15px';
         francoRow.appendChild(francoLabelCell);
         ...
         days.forEach(d => {
             ...
             td.textContent = francos > 0 ? francos : '-';
         ...
         areaCollabs.forEach(collab => {
            const tr = document.createElement('tr');
            let html = `<td class="collab-cell collab-cell-sticky" style="z-index: 20; border-right: 2px solid var(--border); position: sticky; left: 0; background: var(--surface);">
                 <div style="font-weight: 500; font-size: 0.85rem; color: var(--text);">${collab.nombre}</div>
                 <div style="font-size: 0.7rem; color: var(--text-muted); display:flex; gap: 4px;">
                   <span>${collab.legajo || ''}</span>
                   <span>• ${collab.contrato || ''}</span>
                 </div>
               </td>`;

            // Horas planificadas vs contrato
            const mondayStr = formatDate(getStartOfWeek(state.currentWeekStart));
            const hoursPlan = (hoursByWeek[collab.id] && hoursByWeek[collab.id][mondayStr]) ? hoursByWeek[collab.id][mondayStr] : 0;
            const hoursContract = getHorasContrato(collab.contrato);
            const domingosTrabajados = getSundaysWorked(collab.id, state.currentWeekStart);

            let diffClass = '';
            if (hoursPlan > hoursContract) diffClass = 'horas-diff-positiva';
            else if (hoursPlan < hoursContract) diffClass = 'horas-diff-negativa';

            html += `
              <td class="collab-cell metrics-cell" style="padding: 0; min-width: 50px; background: rgba(0,0,0,0.15); border-right: 2px solid var(--border);">
                 <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%;">
                    <div style="font-weight: 700; font-size: 0.75rem; color: var(--text); line-height: 1.1;" class="horas-planificadas-cell" data-collab="${collab.id}">
                       ${hoursPlan.toFixed(1)}<span style="font-size: 0.6rem; color: var(--text-muted);">/${hoursContract}</span>
                    </div>
                    <div style="font-size: 0.65rem; font-weight: 600; margin-top: 2px;" class="${diffClass}">
                       ${hoursPlan !== hoursContract ? (hoursPlan > hoursContract ? '+' : '') + (hoursPlan - hoursContract).toFixed(1) : ''}
                    </div>
                 </div>
              </td>
            `;
            ...
            // Sundays indicator
            const isFrancoError = francoCountByWeek[mondayStr] > 1;
            let francoBg = isFrancoError ? 'background: rgba(220, 38, 38, 0.15);' : 'background: rgba(0,0,0,0.15);';
            let francoColor = isFrancoError ? 'color: #f87171;' : 'color: var(--text-muted);';
            html += `
              <td class="collab-cell metrics-cell" style="padding: 0; min-width: 45px; ${francoBg} border-right: 2px solid var(--border);">
                 <div style="display: flex; align-items: center; justify-content: center; height: 100%;">
                    <div style="font-weight: 600; font-size: 0.75rem; ${francoColor}" title="Domingos trabajados este mes">
                       ${domingosTrabajados}/2
                    </div>
                 </div>
              </td>
            `;
            ...
            days.forEach(d => {
              ...
              let isHoliday = state.holidays.includes(dStr);
              let isTiendaCerrada = isHoliday && state.eventos[dStr] && state.eventos[dStr].descripcion.toLowerCase().includes('cerrad');
              ...
              if (d.getDay() === 0) {
                 ...
                 const nextTargetD = new Date(nextDayStr + "T00:00:00");
                 for (let vac of state.vacations) {
                    if (vac.colaboradorId === collab.id) {
                       const vacStart = new Date(vac.startDate + "T00:00:00");
                       if (nextTargetD.getTime() === vacStart.getTime()) {
              ...
              let inputClass = '';
              let isDisabled = !userHasAccess || isTiendaCerrada || isMockMode;
              let vacationTagHtml = '';
              let parsed = null;

              if (isPreVacationSunday) {
                 inputClass = 'input-absence vacation-active';
                 isDisabled = true;
              } else {
    ...
    async function handleInputChange(e) {
      ...
      if (!validation.valid) {
        if (validation.type === 'legal') {
           showToast("ERROR CRÍTICO DE LEY", `No se puede asignar este horario.<br>El colaborador no cumple con las ${validation.req} de descanso obligatorio por Ley.`);
    ...
    document.getElementById('configBtn').addEventListener('click', () => {
      ...
      if (invContainer) renderGestionInvitados(invContainer);
    });
    ...
    async function fetchHistorialVacaciones(collabId) {
       const q = query(collection(db, "vacaciones"), where("colaboradorId", "==", collabId));
       const snap = await getDocs(q);
    ...
    async function renderSaldosVacaciones() {
       ...
       container.innerHTML = '<div style="padding: 20px; text-align: center; color: var(--text-muted);">Cargando historial...</div>';
    ...
    window.renderVacationCalendar = function() {
       ...
       for (let i = 0; i < 6; i++) {
          ...
          for (let day = 1; day <= daysInMonth; day++) {
             ...
             currentWeek.push({ day, dStr, onVacation, onVacationIds, isHoliday: state.holidays.includes(dStr) });
    ...
    window.showVacationDetails = function(dStr, idsStr) {
       ...
       let detailsHTML = `<h4 style="margin: 0 0 12px 0; color: #fff; font-size: 1rem;">Vacaciones del ${dStr.split('-').reverse().join('/')}</h4>`;
    ...
    window.checkAccessWithToast = function(permiso) {
      if (!checkAccess(permiso)) {
         showToast("Acceso denegado", "No tenés permiso para realizar esta acción.", "warning");
         return false;
      }
      return true;
    };
    ...
    if (localStorage.getItem('adminLogged') === 'true') {
       currentRole = 'admin';
    } else if (localStorage.getItem('invitadoLegajo')) {
       currentRole = 'invitado';
       currentInvitadoLegajo = localStorage.getItem('invitadoLegajo');
    } else if (localStorage.getItem('editorLegajo')) {
       currentRole = 'editor';
    }

    // Inicializar módulo de permisos (inyectando todas las dependencias)
    initPermissionsModule({
      db, state, isMockMode,
      showToast,
      getCurrentRole:   () => currentRole,
      getCurrentLegajo: () => currentInvitadoLegajo,
      setDoc, doc, getDoc, getDocs, collection,
    });
    ...
    function checkLogin() {
       ...
       // Se muestran todas las pestañas, las restricciones dentro se manejan con syncPermissionsUI
       ...
       if (currentRole === 'admin') {
          ...
          // Invitado: acceso limitado según permisos de Firebase
          adminLoginBtn.style.display = 'none';
          logoutBtn.style.display = 'inline-flex';
          logoutBtn.innerHTML = 'Cerrar Sesión';
          // Las restricciones granulares las aplica applyUIRestrictions()
       ...
       // Aplicar restricciones de UI según permisos del usuario activo
       syncPermissionsUI();
       // Botón de eventos: visible solo para admin o quien tiene gestionarEventos
       const eventosBtn = document.getElementById('eventosNavBtn');
       if (eventosBtn) {
           if (currentRole === 'visitor') {
               eventosBtn.style.display = 'none';
           } else {
               eventosBtn.style.display = checkAccess('gestionarEventos') ? 'flex' : 'none';
           }
       }
    };
    ...
    function handleContextMenu(e) {
       ...
       if (eventoHoy) {
         ...
         document.getElementById('ctxInventarioLabel').textContent = `${eventoHoy.tipo}: ${eventoHoy.descripcion}`;
         document.getElementById('cellInventarioInput').checked = !!obj.esInventario;
       } else {
    ...
    document.getElementById('cellInventarioInput').addEventListener('change', async (e) => {
      ...
      if (!isMockMode) {
        try {
          setDoc(doc(db, "planificacion", `${collabId}_${dateStr}`), {
            colaboradorId: collabId,
            fecha: dateStr,
            slot: obj.slot,
    ...
    function resetLoginModal() {
       ...
       document.getElementById('loginModalSubtitle').textContent = 'Ingresá tu número de legajo.';
    ...
    document.getElementById('loginLegajo').addEventListener('input', (e) => {
       ...
       const nomDisplay  = document.getElementById('loginNombreDisplay');
       const nomHidden   = document.getElementById('loginNombreHidden');
       const submitBtn   = document.getElementById('loginSubmitBtn');

       // Resetear campos secundarios
       passWrapper.style.display = 'none';
       nomDisplay.style.display  = 'none';
       ...
       if (leg === ADMIN_LEGAJO) {
          // ── Admin: mostrar campo de contraseña
       ...
       } else if (INVITADOS_AUTORIZADOS[leg]) {
          // ── Invitado autorizado: mostrar nombre
          const nombre = INVITADOS_AUTORIZADOS[leg];
          title.textContent    = 'Acceso Invitado';
          title.style.color    = 'var(--info)';
          subtitle.textContent = 'Acceso con permisos configurados por el Administrador.';
          nomDisplay.textContent   = nombre;
          nomDisplay.style.display = 'block';
          nomHidden.value          = nombre;
          submitBtn.textContent    = 'Ingresar';

       } else {
          // ── Colaborador operativo: buscar en state
          const collab = state.collaborators.find(c =>
             String(c.id).trim() === leg || String(c.legajo).trim() === leg
          );
          if (collab && collab.name) {
             title.textContent    = 'Firma de Edición';
             title.style.color    = 'var(--info)';
             subtitle.textContent = 'Identificado como colaborador operativo.';
             nomDisplay.textContent   = collab.name;
             nomDisplay.style.display = 'block';
             nomHidden.value          = collab.name;
             submitBtn.textContent    = 'Firmar';
          } else {
             title.textContent    = 'Acceso';
             title.style.color    = 'var(--danger)';
             subtitle.textContent = 'Legajo no encontrado en el sistema.';
             submitBtn.textContent = 'Ingresar';
          }
       }
    });
    ...
    document.getElementById('loginForm').addEventListener('submit', async (e) => {
       ...
       const nom  = document.getElementById('loginNombreHidden').value.trim();

       if (!leg) return;

       if (leg === ADMIN_LEGAJO) {
          // ── Flujo Admin
          if (pass === 'AdminGDN2026') {
          ...
          // ── Flujo Invitado
          if (!nom) { showToast('Error', 'Legajo inválido.'); return; }
          const permisosData = await loadPermisosInvitado(leg);
          if (permisosData && permisosData.activo === false) {
             showToast('Acceso denegado', 'Tu cuenta está inactiva. Contactá al Administrador.');
             return;
          }
          localStorage.setItem('invitadoLegajo', leg);
          localStorage.setItem('userLoggedIn', 'true');
          localStorage.setItem('invitadoNombre', nom);
          currentRole = 'invitado';
          currentInvitadoLegajo = leg;
          state.currentInvitadoLegajo = leg;
          loginModal.style.display = 'none';
          checkLogin();
          showToast('Bienvenido/a', nom);

       } else {
          // ── Flujo Editor clásico
          if (!nom) { showToast('Error', 'Legajo no encontrado.'); return; }
       ...
       // Si habia una celda pendiente de editar, enfocarla
    ...
    document.getElementById('editorLegajo').addEventListener('input', (e) => {
       ...
       if (leg) {
          // Primero buscar en la lista fija de invitados autorizados
          let editorName = INVITADOS_AUTORIZADOS[leg];

          // Si no está en invitados, buscar en colaboradores del estado
          if (!editorName) {
    ...
    document.getElementById('editorForm').addEventListener('submit', async (e) => {
       ...
       if (nom && leg) {
          // ── Detección de rol por lista fija (sin consulta Firebase) ──
          const esInvitado = Object.prototype.hasOwnProperty.call(INVITADOS_AUTORIZADOS, leg);
          ...
          if (esInvitado) {
             ...
             if (permisosData && permisosData.activo === false) {
                showToast('Acceso denegado', 'Tu cuenta de invitado está inactiva. Contactá al Administrador.');
                return;
             }
             // Si no existe documento en Firebase aún, el invitado accede sin permisos
             // (el Admin puede configurarlos después)
             localStorage.setItem('invitadoLegajo', leg);
             localStorage.setItem('invitadoNombre', nom);
             currentRole = 'invitado';
             currentInvitadoLegajo = leg;
             editorModal.style.display = 'none';
             checkLogin();
             showToast('Acceso Invitado', `Bienvenido/a, ${nom}`);
          } else {
             // Es un colaborador operativo → flujo de editor clásico
             localStorage.setItem('editorNombre', nom);
             localStorage.setItem('editorLegajo', leg);
             currentRole = 'editor';
             editorModal.style.display = 'none';
    ...
    window.requireEditor = function(e) {
       if (currentRole === 'visitor') {
          ...
          // Abrir el modal unificado de acceso
          resetLoginModal();
          loginModal.style.display = 'flex';
          setTimeout(() => document.getElementById('loginLegajo').focus(), 80);
       } else if (currentRole === 'invitado') {
    ...
    window.logAudit = async function(action, collabId, targetDate, oldValue, newValue) {
       ...
       try {
          ...
          else if (currentRole === 'editor') authorName = `${localStorage.getItem('editorNombre')} (${localStorage.getItem('editorLegajo')})`;
          else if (currentRole === 'invitado') authorName = `${localStorage.getItem('invitadoNombre')} (${localStorage.getItem('invitadoLegajo')}) [Invitado]`;
    ...
    window.renderSugeridos = async function() {
       ...
       if (!dateInput.value) {
          ...
          const localISOTime = (new Date(tomorrow - tzOffset)).toISOString().slice(0, -1).split('T')[0];
          dateInput.value = localISOTime;
       }
       
       const selectedDateStr = dateInput.value;
       const tbody = document.getElementById('sugeridosTableBody');
       tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; padding:20px; color:var(--text-muted);">Cargando sugerencias...</td></tr>';
       ...
       if (typeof isMockMode === 'undefined' || !isMockMode) {
          try {
             ...
             const q = query(collection(db, "sugerencias"), where("fecha", "==", selectedDateStr));
             const snap = await getDocs(q);
             snap.forEach(d => {
             ...
             if (globalNotesTA) {
                globalNotesTA.value = globalDocSnap.exists() ? (globalDocSnap.data().texto || '') : '';
                const canEdit = checkAccess('modificarSugeridos') && currentRole !== 'visitor';
       ...
       console.log("Intentando renderizar tarjetas móviles. isMobile:", isMobile, "cardsContainer:", !!cardsContainer);
       
       state.collaborators.forEach(collab => {
          const isVac = state.vacations && state.vacations.some(vac => 
          ...
          const authorLabel = savedData && savedData.legajo ? `<div style="font-size: 0.7rem; color: var(--text-muted); margin-top: 6px; text-align: right;">Escrito por: <span style="color: var(--primary); font-weight: bold;">${savedData.legajo}</span></div>` : '';
       ...
       document.querySelectorAll('.sugeridos-comment').forEach(textarea => {
          textarea.addEventListener('blur', async (e) => {
             ...
             if (val !== originalText && !e.target.readOnly && !e.target.disabled) {
                ...
                try {
                   await setDoc(doc(db, "sugerencias", `${selectedDateStr}_${collabId}`), commentData);
                   // Opcional: mostrar un mini toast visual para confirmar guardado
                   if (typeof showToast === 'function') {
                      showToast('Guardado', 'Comentario de sugerencia guardado con éxito.', 'success');
                   }
    ...
    document.getElementById('suggestedDateFilter')?.addEventListener('change', window.renderSugeridos);
    
    document.getElementById('globalNotesTextarea')?.addEventListener('blur', async (e) => {
       const val = e.target.value;
       const dateVal = document.getElementById('suggestedDateFilter').value;
       if (!dateVal || !checkAccessWithToast('modificarSugeridos') || currentRole === 'visitor') return;
       try {
          await setDoc(doc(db, "notas_globales", dateVal), { texto: val, fecha: dateVal, timestamp: new Date().toISOString() });
       } catch(err) {
    ...
    document.getElementById('pdfSugeridosBtn')?.addEventListener('click', () => {
       ...
       textareas.forEach(ta => {
           ...
           div.style.cssText = "padding: 2px; font-size: 9.5pt; line-height: 1.15; word-break: break-all; color: black;";
       ...
       if (globalTA) {
           ...
           globalDiv.style.cssText = "margin-top: 15px; padding: 10px; border: 1px solid #000; border-radius: 4px; font-size: 10pt; line-height: 1.2; word-break: break-all; color: #000000 !important; page-break-inside: avoid;";
